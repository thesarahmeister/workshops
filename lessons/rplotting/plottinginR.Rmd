...
# the facets variable takes two arguments: one on the left an one on the right and they are separated by a tilde. The one on the right determines the columns of the facets and the one on the right determines the rows. 
...
```{r}
cutpoints <- quantile(mpg$cty, prob = seq(0,1,length = 4), na.rm=TRUE) # split into tertiles. the length is 4 as 0 is included. 
...
melted <- melt(mpg, id.vars=c("class", "drv"), measure.vars = "displ")
means <- ddply(melted, c("class", "drv"), summarise,
               mean=mean(value))

ggplot(means, aes(x=drv, y=means, fill=class)) + 
    geom_bar(position=position_dodge(), stat="identity") +
    geom_errorbar(aes(ymin=means$mean-mpgsum$se, ymax=means$mean+mpgsum$se), width=.2,position=position_dodge(.9))
