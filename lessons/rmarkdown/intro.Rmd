...
infront like ` `r paste('testing')` `, which creates `r paste('testing')`.
...
```{r fig, fig.height=5, fig.width=5, dpi=90, fig.cap='Scatterplot of speed and distance.'}
plot(cars)
```
...
Now we can cite them in-text, using the ` `r figNums('figCars', display = 'cite')` `, 
which then looks like `r figNums('figCars', display = 'cite')`. And with the 
plot, include this option in the plot code chunk ` `r figNums('figCars')` `:

```{r plotCap, fig.cap=figNums('figCars')}
plot(cars)
```

Or with a table using ` `r tabNums('tabCars', display = 'cite')` ` to show 
`r tabNums('tabCars', display = 'cite')`.
...
## Sharing your code/file

convert to word for oldies (fogie?) (or html)
...
Prettier code for others to see (reformat code in RStudio)

Make sense, so write out what you are doing in the Rmd part and the code in the
code chunk part

# Generating this `.Rmd` document
